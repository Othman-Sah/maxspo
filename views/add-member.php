...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($firstName)) {
        ...
        try {
            ...
            // Auto-calculate amount (Activity Price * 12)
            $priceStmt = $conn->prepare("SELECT monthlyPrice FROM activities WHERE name = ?");
            $priceStmt->execute([$sport]);
            $basePrice = $priceStmt->fetchColumn() ?: 0;
            $totalAmount = $basePrice * 12;

            // Record the yearly payment to update graphs
            $payStmt = $conn->prepare("INSERT INTO payments (member_id, amount, date, method, status, sport) VALUES (?, ?, ?, ?, ?, ?)");
            $payStmt->execute([$memberId, $totalAmount, $joinDate, 'especes', 'valide', $sport]);
                                        ...
                                        <select name="sport" required class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 rounded-2xl outline-none transition-all font-bold text-slate-700">
                                            ...
                                            <?php foreach ($activities as $activity): ?>
                                                <option value="<?php echo htmlspecialchars($activity['name']); ?>">
                                                    <?php echo htmlspecialchars($activity['name']); ?> (<?php echo $activity['monthlyPrice']; ?> DH / An)
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-xl">
                                        ...
                                        <span class="font-black text-slate-900">1 an</span>
                                    </div>
                                </div>
