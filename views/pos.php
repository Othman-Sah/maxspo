    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'process_sale':
                ...
                $db->conn->exec("CREATE TABLE IF NOT EXISTS payments (
                ...
                $payStmt = $db->conn->prepare("INSERT INTO payments (member_id, amount, date, method, status, sport) VALUES (?, ?, ?, ?, ?, ?)");
                $payStmt->execute([0, $total, date('Y-m-d'), $paymentMethod, 'valide', 'Vente POS']);
            ...
            <div class="flex flex-col lg:flex-row gap-8 h-full">
                <div class="flex-1 flex flex-col p-8 overflow-hidden">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0 mb-6 animate-in">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="relative">
                                ...
                                <input type="text" id="search-input" placeholder="Rechercher produit..." class="pl-12 pr-4 py-3 bg-white border border-slate-100 rounded-2xl outline-none focus:border-indigo-500 font-bold text-sm w-full md:w-64 shadow-sm text-slate-900 placeholder-slate-400" />
                            </div>
                    ...
                    <div id="category-filters" class="flex gap-2 overflow-x-auto pb-4 shrink-0 no-scrollbar animate-in" style="animation-delay: 0.1s;">
                        <button data-category="all" class="px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all whitespace-nowrap border bg-slate-900 text-white border-slate-900 shadow-lg hover:bg-slate-800">Tout</button>
                    ...
                    <div id="product-grid" class="flex-1 overflow-y-auto pr-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 animate-in" style="animation-delay: 0.2s;">
                        ...
                        <?php endforeach; ?>
                         <button onclick="openAddProductModal()" class="p-5 rounded-[28px] border-2 border-dashed border-slate-200 text-slate-300 hover:border-indigo-300 hover:bg-indigo-50 transition-all flex flex-col items-center justify-center text-center gap-3 py-12">
                ...
                <div class="w-full lg:w-96 bg-white rounded-[40px] border border-slate-100 shadow-2xl flex flex-col overflow-hidden animate-in" style="animation-delay: 0.3s;">
                    ...
                    <div class="p-8 bg-slate-50 border-t border-slate-100 space-y-6">
                        <div class="space-y-3">
                             ...
                             <div class="flex justify-between text-indigo-600 text-xs font-bold uppercase tracking-widest">
                                <span>Remise</span>
                                <span>0 DH</span>
                             </div>
                             <div class="pt-3 border-t border-slate-200 flex justify-between items-baseline">
                                <span class="text-sm font-black text-slate-900">TOTAL À PAYER</span>
                                <span class="text-4xl font-black text-emerald-400"><span id="cart-total">0</span> <span class="text-xl">DH</span></span>
                             </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                             <button onclick="processPayment('cash')" class="flex flex-col items-center justify-center p-4 bg-white border border-slate-100 hover:bg-slate-50 rounded-2xl transition-all group shadow-sm">
                                <span class="text-emerald-400 group-hover:scale-110 transition-transform text-2xl mb-1">💵</span>
                                <span class="text-[10px] font-black uppercase text-slate-600">Espèces</span>
                             </button>
                        ...
                        <div class="flex items-center justify-center gap-2 text-[10px] font-black uppercase text-slate-300 tracking-tighter">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function filterProducts() {
                ...
                const activeCategory = (categoryFilters.querySelector('.bg-slate-900') || categoryFilters.querySelector('.bg-indigo-600')).dataset.category;
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onsubmit="addProduct(event)" class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Catégorie</label>
                    <select name="category" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                ...
                <div>
                    ...
                    <input type="number" name="price" step="0.01" min="0" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Stock</label>
                    <input type="number" name="stock" min="0" value="0" class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
    ...
    <div id="editProductModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onsubmit="saveProductEdit(event)" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nom</label>
                    <input type="text" id="editProductName" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Catégorie</label>
                    <select id="editProductCategory" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                        <option value="complement">Complément</option>
                        <option value="snack">Snack</option>
                        <option value="boisson">Boisson</option>
                    </select>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Prix (DH)</label>
                    <input type="number" id="editProductPrice" step="0.01" min="0" required class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Stock</label>
                    <input type="number" id="editProductStock" min="0" class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white text-slate-900">
                </div>
                <div class="flex gap-2 pt-4">
